...
export default function ExportDialog({
  ...
  const handleExport = async (format: 'csv' | 'json' | 'zip') => {
    if (results.length === 0) {
      alert('রপ্তানির জন্য কোনো ফলাফল নেই');
      return;
    }
    ...
    try {
      if (format === 'csv') {
        exportToCSV(results);
        alert('CSV হিসেবে রপ্তানি সফল হয়েছে');
      } else if (format === 'json') {
        exportToJSON(results);
        alert('JSON হিসেবে রপ্তানি সফল হয়েছে');
      } else if (format === 'zip') {
        alert('ZIP হিসেবে রপ্তানি শীঘ্রই আসছে');
      }
    } catch (error) {
      alert('রপ্তানিতে ত্রুটি হয়েছে');
      console.error(error);
    } finally {
